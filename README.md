...
| `--assume-valid-mmio` | a register access through an `ioremap`-family **MMIO mapping** (`ioremap`, `of_iomap`, `devm_platform_ioremap_resource`, … — the return is labelled `iomem`) stays within the device's mapping | the mapping's extent is the device's, known only at the mapping site, so a **symbolic** register offset could genuinely overrun it (a real driver bug). Prove-only: never refutes, so with the flag off the access stays UNKNOWN | `valid-mmio` |
| `--assume-field-invariants` | a scalar **loaded from memory** (a struct field / heap slot) holds a value valid for its use — a shift amount below the bit width, a non-zero divisor (`reg->data_size ∈ {1,2,4,8}` feeding `MAKE_64BIT_MASK`) | an opaque write could store an out-of-range value the loaded scalar then feeds to a shift/divide. Prove-only: never refutes, so with the flag off the operation stays UNKNOWN. The scalar analogue of `--assume-valid-params` for loaded pointers | `field-invariants` |
