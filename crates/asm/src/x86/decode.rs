...
pub(crate) fn parse_prefixes(code: &[u8], p: &mut usize) -> csolver_core::Result<(bool, bool, bool, bool, bool, bool, u8)> {
    let mut rex_w = false;
    let mut rex_r = false;
    let mut rex_x = false;
    let mut rex_b = false;
...
pub(crate) fn read_modrm(code: &[u8], at: usize, rex_r: bool, rex_b: bool) -> csolver_core::Result<TypedModRm> {
...
pub(crate) fn read_mem(code: &[u8], p: &mut usize, m: &TypedModRm, rex_x: bool, rex_b: bool) -> csolver_core::Result<Mem> {
    let rm_low = m.rm & 7;
    let mut base = m.rm;
    let mut index = None;
...
#[allow(missing_docs)]
pub(crate) enum Group1Op { Add = 0, Or = 1, Adc = 2, Sbb = 3, And = 4, Sub = 5, Xor = 6, Cmp = 7 }

pub(crate) fn group1_op_from_modrm_reg(code: &[u8], at: usize, rex_r: bool, _rex_b: bool) -> csolver_core::Result<Group1Op> {
