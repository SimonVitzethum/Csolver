...
impl OperandSrc {
    ...
    fn value_arg(self) -> Option<usize> {
        match self {
...
fn asm_operand_layout(constraints: &str) -> AsmLayout {
...
fn asm_mnem_root<'a>(m: &'a str, root: &str) -> Option<&'a str> {
    if m == root {
        return Some(m);
    }
...
fn asm_binop_code(m: &str) -> Option<(char, bool)> {
...
fn asm_reg_semantic(template: &str, constraints: &str, intel: bool) -> Option<String> {
