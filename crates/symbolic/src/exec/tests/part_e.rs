...
fn indirect_call_devirtualised_to_a_pure_summary_preserves_state() {
    ...
    let r = discharge_inner(
        ...
        &empty_grants, &table, None, &HashMap::new(),
    );
    assert!(
    ...
    let r2 = discharge_inner(
        &f, ExecLimits::default(), &summaries, &HashMap::new(), &[], &[], &[], &globals,
        &empty_grants, &HashMap::new(), None, &HashMap::new(),
    );
    assert!(
...
fn indirect_call_through_null_fn_ptr_is_refuted() {
    ...
    let r = discharge_inner(
        ...
        &HashMap::new(), &empty_grants, &HashMap::new(), None, &HashMap::new(),
    );
    let d = r
...
fn write_to_constant_global_is_refuted() {
    ...
    let run = |f: &Function| {
        discharge_inner(
            ...
            &HashMap::new(), &[], &[], &[], &globals, &empty, &HashMap::new(), None, &HashMap::new(),
        )
    };
    let ro = run(&mk("ro"));
