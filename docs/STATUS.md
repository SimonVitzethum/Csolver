...
proved (`ptr_walk_loop_oob` → not PASS).

The **rotated `-O` (bottom-test) form** — where the load precedes the `next ==
end` check — now verifies too. There the bound is the stronger `iter + stride ≤
end`, sound only because the loop is entered non-empty under an `is_empty`
preheader guard. Rather than analyse that guard structurally (and trust it), the
engine **proves the base case** `b0 + stride ≤ end_off` from the guard sitting in
this header's path condition (`base != end`): if the guard is absent, the base
case is unprovable, the bounded offset is never installed, and the load is not
proved — so a rotated walk over a *symbolic*-length buffer verifies **with** the
guard (`ptr_walk_bottom_loop` → PASS) and is correctly not proved **without** it
